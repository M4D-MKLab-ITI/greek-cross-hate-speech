...
# Code from https://github.com/M4D-MKLab-ITI/Domain-aligned-Data-Augmentation
...
def get_task_processor(task, data_dir):
    ...
    if task == 'stsa':
        ...
        return JSONDataProcessor(data_dir=data_dir, skip_header=False, label_col=0, text_col=1)
    else:
...
class DatasetProcessor(object):
    ...
    def _read_json(cls, input_file, quotechar=None):
        """Read a json file """
        # label_resolver = {"1":"Hate","0":"Neutral"}
        label_resolver = {1:"Hate",0:"Neutral"}
        with open(input_file, "r") as f:
            dataset = json.load(f)
            data = dataset['data']
        ...
        for instance in data: 
            lines.append([label_resolver[instance["label"]], instance["text"]])
        return lines

class JSONDataProcessor(DatasetProcessor):
    """Processor for dataset to be augmented from JSON files"""
    ...
    def get_train_examples(self):
        ...
        return self._create_examples(
            self._read_json(os.path.join(self.data_dir, "train.json")), "train")

    def get_dev_examples(self):
        """See base class."""
        return self._create_examples(
            self._read_json(os.path.join(self.data_dir, "test.json")), "dev")

    def get_labels(self, task_name):
        """add your dataset here"""
        labels =  set(["Hate","Neutral"])
